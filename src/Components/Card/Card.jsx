...
function Card() {
  ...
  let { data } = useQuery('getDetails', () => getDetails(id));
      ...
      <div className="container w-4/5 m-auto ">
        <div class="grid grid-cols-3 gap-3">
          ...
          <div className='col-span-2 p-5 bg-amber-800'>
            ...
            {/* <ul>
              <li>publisher {data?.data.volumeInfo.publisher}</li>
              <li>First publish {data?.data.volumeInfo.publishedDate}</li>
              <li>Isbn {data?.data.volumeInfo?.industryIdentifiers && (
                <span className=" mb-2"> {data.data?.volumeInfo?.industryIdentifiers?.map((identifier) => (
                  <span key={identifier.identifier} className="text-md">
                    {identifier.identifier} &nbsp;
                  </span>
                ))}</span>
              )}</li>
              <li>Language {data?.data.volumeInfo.language}</li>
              <li>Pages {data?.data.volumeInfo.pageCount}</li>
              <li>printed Pages {data?.data.volumeInfo.printedPageCount}</li>
            </ul>
            <div className="">Categories &nbsp;&nbsp;&nbsp; {data?.data.volumeInfo.categories}</div> */}
          </div>
        </div>
      </div>
    </>
